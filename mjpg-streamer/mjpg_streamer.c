...
void forkThreadForMonitorCommand() {
    int createThreadResult;
    createThreadResult = pthread_create(&commandThread, NULL, startMonitorCommand, NULL);
    if (createThreadResult) {
        printf("Try Create Sub Thread startMonitorCommand Failed.\n");
    } else {
        printf("Sub Thread startMonitorCommand Started.\n");
    }
}
*/
void* startCommandServer(void *ptr) {
    int canCancel = pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
    if (canCancel != 0) {
        printf("This Poke Thread can not be cancelled.\n");
    }
    system("python /home/pi/serverWebPython.py 8000");
    return NULL;
}
void forkThreadForCommandServer() {
    int createThreadResult;
    createThreadResult = pthread_create(&commandThread, NULL, startCommandServer, NULL);
    if (createThreadResult) {
        printf("Try Create Sub Thread startCommandServer Failed.\n");
    } else {
        printf("Sub Thread startCommandServer Started.\n");
    }
}
...
int main(int argc, char *argv[])
{
    ...
    int daemon = 0, i;
    size_t tmp = 0;
    //forkThreadForMonitorCommand();
    forkThreadForCommandServer();
